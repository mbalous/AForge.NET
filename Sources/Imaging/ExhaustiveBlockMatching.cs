...
namespace AForge.Imaging
{
    ...
    public class ExhaustiveBlockMatching : IBlockMatching
    {
        ...
        /// 
        public List<BlockMatch> ProcessImage(BitmapData sourceImageData, List<IntPoint> coordinates,
        ...
        /// <exception cref="ArgumentException">Source and search images must have same pixel format.</exception>
        /// 
        public List<BlockMatch> ProcessImage(UnmanagedImage sourceImage, List<IntPoint> coordinates,
            UnmanagedImage searchImage)
        {
            ...
            if ((sourceImage.PixelFormat != PixelFormat.Format8bppIndexed) &&
                (sourceImage.PixelFormat != PixelFormat.Format24bppRgb))
                throw new UnsupportedImageFormatException(
                    "Source images can be graysclae (8 bpp indexed) or color (24 bpp) image only");
            ...
            unsafe
            {
                ...
                for (int iPoint = 0; iPoint < pointsCount; iPoint++)
                {
                    ...
                    {
                        if ((searchStartY + searchWindowRow < 0) ||
                            (searchStartY + searchWindowRow + blockSize >= height))
                        {
                            // skip row
                            continue;
                        }
                        ...
                        for (int searchWindowCol = 0; searchWindowCol < searchWindowSize; searchWindowCol++)
                        {
                            ...
                            byte* ptrSourceBlock = ptrSource + ((refPointY - blockRadius)*stride) +
                                                   ((refPointX - blockRadius)*pixelSize);
                            ...
                            {
                                for (int blockCol = 0;
                                    blockCol < blockLineSize;
                                    blockCol++, ptrSourceBlock++, ptrSearchBlock++)
                                {
