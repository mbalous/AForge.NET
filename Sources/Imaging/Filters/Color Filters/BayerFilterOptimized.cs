        ...
        protected override void ProcessFilter(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
                ...
                (width < 2) || (height < 2))
            {
                throw new InvalidImagePropertiesException(
                    "Source image must have even width and height. Width and height can not be smaller than 2.");
            }
        ...
        private unsafe void ApplyBGGR(UnmanagedImage sourceData, UnmanagedImage destinationData)
        {
            ...
            for (int y = 1; y < heightM1; y += 2)
            {
                ...
                for (int x = 1; x < widthM1; x += 2)
                {
                    ...
                    dst[RGB.B] =
                        (byte) ((src[srcMStrideM1] + src[srcMStrideP1] + src[srcStrideM1] + src[srcStrideP1]) >> 2);
                    ...
                    dst[RGB.R] = (byte) ((src[srcMStride] + src[srcStride]) >> 1);
                    dst[RGB.G] =
                        (byte) ((*src + src[srcMStrideM1] + src[srcMStrideP1] + src[srcStrideM1] + src[srcStrideP1])/5);
                    dst[RGB.B] = (byte) ((src[-1] + src[1]) >> 1);
                    ...
                    dst[RGB.R] = (byte) ((src[-1] + src[1]) >> 1);
                    dst[RGB.G] =
                        (byte) ((src[srcMStrideM1] + src[srcMStrideP1] + src[srcStrideM1] + src[srcStrideP1] + *src)/5);
                    dst[RGB.B] = (byte) ((src[srcMStride] + src[srcStride]) >> 1);
