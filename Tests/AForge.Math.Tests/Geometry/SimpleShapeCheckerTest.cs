...
namespace AForge.Math.Geometry.Tests
{
    ...
    {
        private readonly SimpleShapeChecker shapeChecker = new SimpleShapeChecker();

        private readonly List<IntPoint> idealCicle = new List<IntPoint>();
        private readonly List<IntPoint> distorredCircle = new List<IntPoint>();

        private readonly List<IntPoint> square1 = new List<IntPoint>();
        private readonly List<IntPoint> square1Test = new List<IntPoint>();
        private readonly List<IntPoint> square2 = new List<IntPoint>();
        private readonly List<IntPoint> square2Test = new List<IntPoint>();
        private readonly List<IntPoint> square3 = new List<IntPoint>();
        private readonly List<IntPoint> rectangle = new List<IntPoint>();

        private readonly List<IntPoint> triangle1 = new List<IntPoint>();
        private readonly List<IntPoint> isoscelesTriangle = new List<IntPoint>();
        private readonly List<IntPoint> equilateralTriangle = new List<IntPoint>();
        private readonly List<IntPoint> rectangledTriangle = new List<IntPoint>();
        ...
        [TestCase(PolygonSubType.Unknown, new[] {0, 0, 100, 0, 90, 10})] // just a triangle
        [TestCase(PolygonSubType.IsoscelesTriangle, new[] {0, 0, 100, 0, 50, 10})]
        [TestCase(PolygonSubType.IsoscelesTriangle, new[] {0, 0, 100, 0, 50, 200})]
        [TestCase(PolygonSubType.EquilateralTriangle, new[] {0, 0, 100, 0, 50, 86})]
        [TestCase(PolygonSubType.RectangledIsoscelesTriangle, new[] {0, 0, 100, 0, 50, 50})]
        [TestCase(PolygonSubType.RectangledIsoscelesTriangle, new[] {0, 0, 100, 0, 0, 100})]
        [TestCase(PolygonSubType.RectangledTriangle, new[] {0, 0, 100, 0, 0, 50})]
        [TestCase(PolygonSubType.Unknown, new[] {0, 0, 100, 0, 90, 50, 10, 70})] // just a quadrilateral
        [TestCase(PolygonSubType.Trapezoid, new[] {0, 0, 100, 0, 90, 50, 10, 50})]
        [TestCase(PolygonSubType.Trapezoid, new[] {0, 0, 100, 0, 90, 50, 0, 50})]
        [TestCase(PolygonSubType.Trapezoid, new[] {0, 0, 100, 0, 90, 50, 0, 53})] // a bit disformed
        [TestCase(PolygonSubType.Parallelogram, new[] {0, 0, 100, 0, 120, 50, 20, 50})]
        [TestCase(PolygonSubType.Parallelogram, new[] {0, 0, 100, 0, 70, 50, -30, 50})]
        [TestCase(PolygonSubType.Rectangle, new[] {0, 0, 100, 0, 100, 50, 0, 50})]
        [TestCase(PolygonSubType.Rectangle, new[] {0, 0, 100, 0, 100, 52, -3, 50})] // a bit disformed
        [TestCase(PolygonSubType.Square, new[] {0, 0, 100, 0, 100, 100, 0, 100})]
        [TestCase(PolygonSubType.Square, new[] {50, 0, 100, 50, 50, 100, 0, 50})]
        [TestCase(PolygonSubType.Square, new[] {51, 0, 100, 49, 50, 101, 1, 50})] // a bit disformed
        [TestCase(PolygonSubType.Rhombus, new[] {30, 0, 60, 50, 30, 100, 0, 50})]
        [TestCase(PolygonSubType.Rhombus, new[] {0, 0, 100, 0, 130, 95, 30, 95})]
        [TestCase(PolygonSubType.Unknown, new[] {0, 0, 100, 0, 90, 50, 40, 70, 10, 40})] // unknown if 5 corners or more
        public void CheckPolygonSubTypeTest(PolygonSubType expectedSubType, int[] corners)
        {
            Assert.AreEqual(expectedSubType, this.shapeChecker.CheckPolygonSubType(GetListOfPointFromArray(corners)));
        }
        ...
        public void CheckQuadrilateralCornersTest()
        {
            ...
            Assert.AreEqual(true, CompareShape(corners, this.square1Test));

            Assert.AreEqual(true, this.shapeChecker.IsQuadrilateral(this.square2, out corners));
            Assert.AreEqual(4, corners.Count);
            Assert.AreEqual(true, CompareShape(corners, this.square2Test));
        }
